...
public class NCUtil {
    ...
    public static Visit eventToVisit(Event event, String visitID) throws JSONException {
        ...
        if (eventDetails != null)
            visit.setVisitGroup(eventDetails.get(Constants.VMMC_VISIT_GROUP));
    ...
    public static void processVmmcVisit(EventClient baseEvent) {
        processVmmcVisit(baseEvent, null);
    }

    public static void processSubVmmcVisit(EventClient baseEvent, String parentEventType) {
        processVmmcVisit(baseEvent, null, parentEventType);
    }

    public static void processSubVmmcVisit(EventClient baseEvent, SQLiteDatabase database, String parentEventType) {
        processVmmcVisit(baseEvent, database, parentEventType);
    }

    public static void processVmmcVisit(EventClient baseEvent, SQLiteDatabase database) {
        processVmmcVisit(baseEvent, database, null);
    }

    public static void processVmmcVisit(EventClient baseEvent, SQLiteDatabase database, String parentEventType) {
        try {
    ...
    public static Visit eventToVisit(org.smartregister.domain.Event event) throws JSONException {
        ...
        Visit visit = new Visit();
        visit.setVisitId(org.smartregister.chw.vmmc.util.VmmcJsonFormUtils.generateRandomUUIDString());
        visit.setBaseEntityId(event.getBaseEntityId());
        visit.setDate(event.getEventDate().toDate());
        visit.setVisitType(event.getEventType());
        visit.setEventId(event.getEventId());
        visit.setFormSubmissionId(event.getFormSubmissionId());
        visit.setJson(new JSONObject(org.smartregister.chw.vmmc.util.VmmcJsonFormUtils.gson.toJson(event)).toString());
        ...
        if (event.getObs() != null) {
            for (org.smartregister.domain.Obs obs : event.getObs()) {
                if (!exceptions.contains(obs.getFormSubmissionField())) {
                    VisitDetail detail = new VisitDetail();
                    detail.setVisitDetailsId(org.smartregister.chw.vmmc.util.VmmcJsonFormUtils.generateRandomUUIDString());
    ...
    public static String getDetailsValue(VisitDetail detail, String val) {
        String clean_val = VmmcJsonFormUtils.cleanString(val);
